...
plt.ylabel("Price")
plt.xlabel("Size")
plt.savefig('display_house_price_01.svg') 
...
with tf.Session() as sess:
   ...
   # keep iterating the training data
    for iteration in range(num_training_iter):
        ...
        for (size, price) in zip(train_house_size_norm, train_price_norm):
            sess.run(optimizer, feed_dict={tf_house_size: size, tf_price: price})
            ...
            print("House size:",round(size, 2), " Original price:", round(price,2) , "Price Prediction:", round(price_prediction, 2), "Diff:", (round(denormalize(price, house_price ) )) )
    ...
    plt.savefig('display_house_price_02.svg')  
    print("TensorFlow Rank:", tf.rank)
    #print("TensorFlow Shape:", tf.get_shape())
